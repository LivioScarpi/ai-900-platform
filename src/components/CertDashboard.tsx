...
import { type CertConfig } from "@/lib/certifications";
...
/**
 * Per-certification dashboard body. All stats are scoped to `certId` via the
 * `cert_id` column on attempts / exam_sessions. Used standalone on
 * /[certId]/dashboard (showHeader) and embedded under the global /dashboard tabs.
 */
export async function CertDashboard({ certId, config, showHeader = true }: {
  certId: string;
  config: CertConfig;
  showHeader?: boolean;
}) {
  ...
  const [attemptsRes, historyRes, questionAttemptsRes] = await Promise.all([
    supabase.from("attempts").select("is_correct").eq("user_id", user.id).eq("cert_id", certId),
    supabase.from("exam_sessions").select("*").eq("user_id", user.id).eq("cert_id", certId).order("created_at", { ascending: false }).limit(10),
    supabase.from("attempts").select("question_id, is_correct").eq("user_id", user.id).eq("cert_id", certId),
  ]);
