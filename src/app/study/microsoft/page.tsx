...
import { getMicrosoftQuestions } from "@/lib/questions";
...
export default function MicrosoftSimulationPage() {
  const userId = useUserId();
  const [questions] = useState<Question[]>(() =>
    [...getMicrosoftQuestions()].sort(() => Math.random() - 0.5)
  );
  ...
  if (questions.length === 0) {
    return (
      <div className="px-4 md:px-8 py-6 md:py-8 max-w-2xl mx-auto w-full">
        <p className="label-caps text-ink-faint mb-1">Study / Microsoft Simulation</p>
        <h1 className="font-display text-3xl font-bold text-ink mb-6">No questions yet</h1>
        <div className="card p-8 text-center">
          <p className="text-ink-muted">Microsoft simulation questions will appear here.</p>
          <Link href="/" className="inline-block mt-4 px-5 py-2.5 rounded-lg bg-brand text-white text-sm font-semibold hover:bg-brand-dark transition-colors">
            Back to Home
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="px-4 md:px-8 py-6 md:py-8 max-w-2xl mx-auto w-full">
        ...
        <div className="card p-8 text-center">
          ...
          <p className="text-ink font-semibold mb-4">You&apos;ve completed all Microsoft simulation questions!</p>
  ...
  return (
    <div className="px-8 py-8 max-w-2xl mx-auto w-full">
      <p className="label-caps text-ink-faint mb-1">Study / <span className="text-ink-muted">Microsoft Simulation</span></p>
